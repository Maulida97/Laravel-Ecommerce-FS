...
@section('title', 'Edit Product — Tokoku.id')

@section('menu-products-active', 'active')

@section('breadcrumb')
    <span>Products</span> / <span>Edit</span>
@endsection
            ...
            <div style="display: flex; flex-direction: column; gap: var(--space-6); min-width: 0;">
                ...
                <div class="form-card">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="name" class="form-input @error('name') error @enderror" id="prodName" value="{{ old('name', $product->name) }}" placeholder="e.g. Cotton T-Shirt" required>
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="slug" class="form-input @error('slug') error @enderror" id="prodSlug" value="{{ old('slug', $product->slug) }}" placeholder="e.g. cotton-t-shirt">
                        @error('slug')
                            <div class="error-msg">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="sku" class="form-input @error('sku') error @enderror" id="prodSku" value="{{ old('sku', $product->sku) }}" placeholder="e.g. TSHIRT-COT-01" required>
                            @error('sku')
                                <div class="error-msg">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="short_description" class="form-input @error('short_description') error @enderror" id="prodShortDesc" value="{{ old('short_description', $product->short_description) }}" placeholder="Brief summary of the product...">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" class="form-textarea @error('description') error @enderror" id="prodDesc" placeholder="Describe your product details here...">{{ old('description', $product->description) }}</textarea>
                        @error('description')
                            <div class="error-msg">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="number" name="price" class="form-input @error('price') error @enderror" id="prodPrice" value="{{ old('price', $product->price) }}" placeholder="e.g. 150000" min="0" required>
                            @error('price')
                                <div class="error-msg">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="compare_at_price" class="form-input @error('compare_at_price') error @enderror" id="prodComparePrice" value="{{ old('compare_at_price', $product->compare_at_price) }}" placeholder="e.g. 200000" min="0">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="number" name="weight" class="form-input @error('weight') error @enderror" id="prodWeight" value="{{ old('weight', $product->weight) }}" placeholder="e.g. 200" min="0" required>
                            @error('weight')
                                <div class="error-msg">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group" id="stockQuantityGroup">
                            ...
                            <input type="number" name="stock_quantity" class="form-input @error('stock_quantity') error @enderror" id="prodStock" value="{{ old('stock_quantity', $product->stock_quantity) }}" placeholder="e.g. 50" min="0">
                ...
                <div class="form-card">
                    ...
                    <div class="image-uploader-container">
                        ...
                        <div class="uploaded-images-grid" id="uploadedImagesGrid">
                            <!-- Existing & New images dynamically rendered -->
                        </div>
                    </div>
                </div>
                ...
                <div class="form-card">
                    <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 8px;">
                        ...
                        <div class="toggle-wrap">
                            <input type="checkbox" name="has_variants" id="hasVariantsInput" value="1" {{ $product->variants->isNotEmpty() ? 'checked' : '' }} style="display: none;">
                            <div class="toggle {{ $product->variants->isNotEmpty() ? 'active' : '' }}" id="hasVariantsToggle" role="switch" aria-checked="{{ $product->variants->isNotEmpty() ? 'true' : 'false' }}" tabindex="0"><div class="toggle-knob"></div></div>
                            <span class="toggle-label" style="font-weight: 600;">This product has variations (e.g. Size, Color)</span>
                        </div>
                    </div>

                    <div class="variants-wrapper" id="variantsWrapper" style="{{ $product->variants->isNotEmpty() ? 'display: block;' : 'display: none;' }}">
            ...
            <div style="display: flex; flex-direction: column; gap: var(--space-6); position: sticky; top: calc(var(--header-height) + 24px);">
                ...
                <div class="form-card">
                    ...
                    <div class="form-group" style="margin-bottom: 20px;">
                        <div class="toggle-wrap">
                            <input type="checkbox" name="is_active" id="isActiveInput" value="1" {{ old('is_active', $product->is_active) == '1' ? 'checked' : '' }} style="display: none;">
                            <div class="toggle {{ old('is_active', $product->is_active) == '1' ? 'active' : '' }}" id="isActiveToggle" role="switch" aria-checked="true" tabindex="0"><div class="toggle-knob"></div></div>
                            <span class="toggle-label" id="isActiveLabel">{{ old('is_active', $product->is_active) == '1' ? 'Active (Visible in catalog)' : 'Inactive (Hidden)' }}</span>
                        </div>
                    </div>

                    <div class="form-group" style="margin-bottom: 24px;">
                        <div class="toggle-wrap">
                            <input type="checkbox" name="is_featured" id="isFeaturedInput" value="1" {{ old('is_featured', $product->is_featured) == '1' ? 'checked' : '' }} style="display: none;">
                            <div class="toggle {{ old('is_featured', $product->is_featured) == '1' ? 'active' : '' }}" id="isFeaturedToggle" role="switch" aria-checked="false" tabindex="0"><div class="toggle-knob"></div></div>
                            <span class="toggle-label" id="isFeaturedLabel">{{ old('is_featured', $product->is_featured) == '1' ? 'Featured product' : 'Not featured' }}</span>
                        </div>
                    </div>
                ...
                <div class="form-card">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="meta_title" class="form-input" id="metaTitle" value="{{ old('meta_title', $product->meta_title) }}" placeholder="Search engine title...">
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="metaDesc">SEO Description</label>
                        <textarea name="meta_description" class="form-textarea" id="metaDesc" style="min-height: 80px;" placeholder="Search engine description...">{{ old('meta_description', $product->meta_description) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    (function() {
        ...
        let isSlugEdited = prodSlug.value.length > 0;
        ...
        const primaryImageIdInput = document.getElementById('primaryImageIdInput');
        const deletedImagesInput = document.getElementById('deletedImagesInput');

        let imageFilesList = []; // Newly added File objects
        let existingImages = {!! json_encode($product->images) !!}; // Database images
        let deletedImageIds = [];

        // Render images grid initially
        renderImagesGrid();
        ...
        function renderImagesGrid() {
            ...
            // 1. Render Existing Database Images first
            existingImages.forEach(img => {
                ...
                removeBtn.addEventListener('click', (e) => {
                    ...
                    // If we delete the primary image, pick a new one
                    if (primaryImageIdInput.value == img.id) {
                        primaryImageIdInput.value = '';
                        // Find first remaining existing image to set primary
                        const remaining = existingImages.find(x => !deletedImageIds.includes(x.id));
                        if (remaining) {
                            primaryImageIdInput.value = remaining.id;
                ...
                const isPrimary = (primaryImageIdInput.value == img.id);
                ...
                primaryBadge.addEventListener('click', () => {
                    primaryImageIdInput.value = img.id;
                    primaryImageIndexInput.value = 0;
                    renderImagesGrid();
                });
            ...
            // 2. Render newly added image files
            imageFilesList.forEach((file, index) => {
                ...
                reader.onload = (e) => htmlImg.src = e.target.result;
                ...
                removeBtn.addEventListener('click', (e) => {
                    ...
                    const targetIndex = parseInt(primaryImageIndexInput.value);
                    if (targetIndex === index) {
                        primaryImageIndexInput.value = 0;
                    } else if (targetIndex > index) {
                        primaryImageIndexInput.value = targetIndex - 1;
                    }
                    renderImagesGrid();
                    updateInputFiles();
                });
                ...
                // Primary Badge selector (only if no existing image is marked primary)
                const isPrimary = (primaryImageIdInput.value === '' && parseInt(primaryImageIndexInput.value) === index);
                const primaryBadge = document.createElement('div');
                primaryBadge.className = `primary-badge ${isPrimary ? 'is-primary' : 'not-primary'}`;
                primaryBadge.textContent = isPrimary ? 'Primary Image' : 'Set as Primary';
                primaryBadge.addEventListener('click', () => {
        ...
        let currentCombinations = {!! $variantsJson !!} || []; // [{ name: 'S - Red', sku: 'SKU-S-RED', price_adjustment: 0, stock_quantity: 10, is_active: true, attributes: [...] }]

        // Re-initialize from existing combinations if present
        initializeVariantsFromDatabase();
        ...
        function initializeVariantsFromDatabase() {
            if (currentCombinations.length === 0) return;

            // Extract unique attributes and their values from currentCombinations
            const attrMap = {};
            currentCombinations.forEach(comb => {
                comb.attributes.forEach(attr => {
            ...
            // Rebuild attributesState
            Object.keys(attrMap).forEach(name => {
                const values = Array.from(attrMap[name]);
                const attrId = Date.now() + Math.random().toString(36).substr(2, 5);
                const attrData = { id: attrId, name: name, values: values };
                attributesState.push(attrData);

                // Render in UI
                renderAttributeRowInUI(attrData);
            });
        ...
        function renderAttributeRowInUI(attrData) {
            const row = document.createElement('div');
            row.className = 'attribute-row';
            row.setAttribute('data-id', attrData.id);

            row.innerHTML = `
                <div class="attribute-row-header">
                    <div style="flex: 1; max-width: 200px;">
                        <select class="form-select attr-name-select" style="height: 36px;" required>
                            <option value="Size" ${attrData.name === 'Size' ? 'selected' : ''}>Size</option>
                            <option value="Color" ${attrData.name === 'Color' ? 'selected' : ''}>Color</option>
                            <option value="Material" ${attrData.name === 'Material' ? 'selected' : ''}>Material</option>
                        </select>
                    </div>
            ...
            row.querySelector('.btn-remove-attribute').addEventListener('click', () => {
                attributesState = attributesState.filter(a => a.id !== attrData.id);
                row.remove();
                regenerateCombinations();
            });
        ...
        function regenerateCombinations() {
            ...
            cartesianCombos.forEach(combo => {
                ...
                const existing = currentCombinations.find(c => {
                    return Object.keys(comboAttributes).every(key => {
                        // Find matching attribute in c.attributes array
                        const found = c.attributes.find(a => a.name === key);
                        return found && found.value === comboAttributes[key];
                    });
                });
