...
@section('title', 'Create Category — Tokoku.id')

@section('menu-categories-active', 'active')

@section('breadcrumb')
    <span>Categories</span> / <span>Create</span>
@endsection
    ...
    <div class="table-container" style="padding: 24px; background: var(--bg-primary);">
        <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data" id="categoryForm">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" class="form-input @error('name') error @enderror" id="name" value="{{ old('name') }}" placeholder="e.g. Electronics" required>
                @error('name')
            ...
            <div class="form-group">
                ...
                <input type="text" name="slug" class="form-input @error('slug') error @enderror" id="slug" value="{{ old('slug') }}" placeholder="e.g. electronics (leave empty to auto-generate)">
                @error('slug')
                    <div class="error-msg">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="description" class="form-textarea @error('description') error @enderror" id="description" placeholder="Brief description...">{{ old('description') }}</textarea>
                @error('description')
                    <div class="error-msg">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" name="sort_order" class="form-input @error('sort_order') error @enderror" id="sort_order" placeholder="1" min="0" value="{{ old('sort_order', 1) }}">
                @error('sort_order')
                    <div class="error-msg">{{ $message }}</div>
                @enderror
            </div>
